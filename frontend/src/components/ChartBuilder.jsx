...
export default function ChartBuilder({ permissions = [], user }) {
  ...
  useEffect(() => {
    setColors(current => {
      const next={...current}; resultSeries.forEach((item,index)=>{ if(!next[item.key]) next[item.key]=PALETTE[index%PALETTE.length] }); return next
    })
  }, [resultSeries])
  useEffect(() => {
  ...
  function resetColors(){ const next={}; resultSeries.forEach((item,index)=>{next[item.key]=PALETTE[index%PALETTE.length]}); setColors(next) }
  ...
  const calculatedMaxBarSize = useMemo(() => Math.min(32, Math.max(6, Math.floor(300 / Math.max(1, resultSeries.length)))), [resultSeries.length])

  return <section className="page custom-chart-builder">
    <div className="page-header">
      <div><p className="eyebrow">Custom Chart Builder</p><h2>สร้างกราฟจากฐานข้อมูล</h2><p className="muted">เลือกช่วงเวลา ประเภท ชนิดย่อย ตัวชี้วัด รูปแบบกราฟ และสีได้โดยไม่แก้ไขข้อมูลต้นทาง</p></div>
    </div>

    <div className="card chart-builder-controls">
      <div className="chart-step-heading"><span>1</span><div><h3>เลือกข้อมูล</h3><p className="muted no-margin">กำหนดช่วงเวลา แล้วเลือกเฉพาะข้อมูลที่ต้องการเปรียบเทียบ</p></div></div>
      <div className="chart-filter-grid">
        ...
        <label className="field"><span>ตัวชี้วัดข้อมูลจริง</span><select value={metric} onChange={e=>setMetric(e.target.value)}>{METRICS.map(([value,label,unit])=><option key={value} value={value}>{label} ({unit})</option>)}</select><small className="muted">ระบบเลือกจำนวนหรือน้ำหนักให้เหมาะกับประเภทข้อมูลอัตโนมัติ</small></label>
      </div>

      <div className="section-title-row chart-series-heading"><div><h3>ประเภทข้อมูล <span className="selection-count">เลือก {selectedCount} ชุด</span></h3><p className="muted no-margin">กดประเภทหลักเพื่อดูและเลือกชนิดย่อย</p></div><div className="inline-actions chart-quick-actions"><button className="ghost" onClick={selectAll}>เลือกทั้งหมด</button><button className="ghost" onClick={clearAll}>เคลียร์ทั้งหมด</button><button className="ghost" onClick={selectOnlyWithData} disabled={!resultSeries.length}>เฉพาะที่มีข้อมูล</button><button className="ghost" onClick={()=>refetch()} disabled={isFetching||!selectedSeries.length}>รีเฟรช</button></div></div>
      <div className="check-grid chart-module-grid">
        {MODULE_ORDER.map(module=><div className="chart-module-choice" key={module}>
          <label className="check-pill"><input type="checkbox" checked={selectedSeries.some(token=>tokenModule(token)===module)} onChange={()=>toggleModule(module)}/><span>{MODULE_LABELS[module]}</span></label>
          {(categoriesByModule[module]||[]).length>0 && selectedSeries.some(token=>tokenModule(token)===module) && <div className="chart-subcategory-list">
            {(categoriesByModule[module]||[]).map(category=><label key={category.code}><input type="checkbox" checked={selectedSeries.includes(categoryToken(module,category.code))} onChange={()=>toggleCategory(module,category.code)}/><span>{category.name_th}</span></label>)}
          </div>}
        </div>)}
      </div>
      <button type="button" className="chart-advanced-toggle" onClick={()=>setShowAdvanced(value=>!value)} aria-expanded={showAdvanced}>{showAdvanced?'ซ่อนตัวเลือกเพิ่มเติม':'ตัวเลือกเพิ่มเติม'} <span>{showAdvanced?'▲':'▼'}</span></button>
      {showAdvanced&&<div className="chart-advanced-panel">
    ...
    {!allZero && !incompatible && <>
      <div className="card custom-chart-preview" data-report-section="chart-builder">
        ...
        <div ref={chartExportRef} className="chart-preview-box large report-chart-frame">
          {chartType==='bar' && <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} maxBarSize={calculatedMaxBarSize} barGap={2} margin={{ top: 20, right: 30, left: 10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label"/><YAxis/><Tooltip formatter={(value,name)=>[formatNumber(value),resultSeries.find(item=>item.key===name)?.label||name]}/><Legend formatter={key=>resultSeries.find(item=>item.key===key)?.label||key}/>{resultSeries.map(item=><Bar key={item.key} dataKey={item.key} fill={colors[item.key]} radius={[4,4,0,0]} maxBarSize={calculatedMaxBarSize}/>)}</BarChart></ResponsiveContainer>}
          {chartType==='line' && <ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 20, right: 30, left: 10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label"/><YAxis/><Tooltip formatter={(value,name)=>[formatNumber(value),resultSeries.find(item=>item.key===name)?.label||name]}/><Legend formatter={key=>resultSeries.find(item=>item.key===key)?.label||key}/>{resultSeries.map(item=><Line key={item.key} dataKey={item.key} name={item.key} stroke={colors[item.key]} strokeWidth={3} dot={{r:3}} type="monotone"/>)}</LineChart></ResponsiveContainer>}
          {chartType==='pie' && <ResponsiveContainer width="100%" height="100%"><PieChart margin={{ top: 20, right: 30, left: 10, bottom: 25 }}><Pie data={resultSeries.filter(item=>item.total!==0)} dataKey="total" nameKey="label" outerRadius={140} label={({label,percent})=>`${label} ${(percent*100).toFixed(0)}%`}>{resultSeries.filter(item=>item.total!==0).map(item=><Cell key={item.key} fill={colors[item.key]}/>)}</Pie><Tooltip formatter={value=>formatNumber(value)}/><Legend formatter={key=>resultSeries.find(item=>item.key===key||item.label===key)?.label||key}/></PieChart></ResponsiveContainer>}
        </div>
