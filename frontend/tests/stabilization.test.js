...
test('Report Studio scales chart and table text, keeps titles unchanged, and supports table colors', () => {
  ...
  assert.match(ledger, /const \[chartSize,setChartSize\]=useState\(initialChartSize\)/)
...
test('Report graph height targets every chart frame and table cells keep one border grid', () => {
  ...
  assert.match(ledger, /onChartSizeChange\?\.\(next\)/)
  assert.match(ledger, /useEffect\(\(\)=>setChartSize\(initialChartSize\),\[initialChartSize\]\)/)
  assert.match(ledger, /requestAnimationFrame/)
  assert.match(ledger, /initialChartSize=\{getSectionGraphHeight\(reportStudioSection\)\}/)
  assert.match(ledger, /Number\.isFinite\(sectionGraphHeights\[section\]\)/)
