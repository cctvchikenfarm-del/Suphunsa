...
test('Monthly report uses an adjustable accumulated range and module-correct chart rules', () => {
  ...
  assert.equal(ledger.includes("compact:{ label:'กะทัดรัดสำหรับครอป'"), true)
  assert.equal(ledger.includes("powerpoint:{ label:'PowerPoint 16:9'"), true)
...
test('Report cards persist bounded drag resizing and PowerPoint-ready presets', () => {
  const ledger = fs.readFileSync(new URL('../src/components/AnnualLedger.jsx', import.meta.url), 'utf8')
  const styles = fs.readFileSync(new URL('../src/styles.css', import.meta.url), 'utf8')
  assert.match(ledger, /startSectionResize/)
  assert.match(ledger, /ckap-ledger-section-widths/)
  assert.match(ledger, /role="separator"/)
  assert.match(ledger, /PowerPoint 16:9/)
  assert.match(ledger, /รายงาน 4:3/)
  assert.match(styles, /\.ledger-resize-handle/)
  assert.match(styles, /--report-font-scale/)
...
test('mobile workspace uses isolated bottom navigation without changing desktop sidebar', () => {
  ...
  assert.match(styles, /\.ledger-resize-handle \{ display:none; \}/)
